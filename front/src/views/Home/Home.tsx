...
const filterTuples: [string, string, string, string] = [
  "Sans filtre",
  "Nom",
  "Sexe",
  "Planète",
...
const Home: React.FunctionComponent = () => {
  ...
  const searchNameAPI = async () => {
    ...
    const getPeopleByName: AxiosResponse = await axios.get(
      `http://localhost:4000/api/v1/people/all?name=${name.newName}`
    );
    ...
    if (getPeopleByName.data.result.length === 5) {
      const countPeople: AxiosResponse = await axios.get(
        `http://localhost:4000/api/v1/people/all?name=${name.newName}&skip=${
          valueSkip + 5
        }`
      );
  ...
  const seeMorePeopleName = async () => {
    ...
    const checkFuturPeople: AxiosResponse = await axios.get(
      `http://localhost:4000/api/v1/people/all?name=${name.newName}&skip=${
        skip + 5
      }`
    );
    ...
    if (checkFuturPeople.data.result.length === 5) setSkip(skip + 5);
      ...
      <div className="container">
        <Header
          ...
          onChangeInput={(e: React.ChangeEvent<HTMLInputElement>) =>
            setName({ newName: e.target.value, oldName: name.newName })
          }
        />
        {!people.result.length ? (
          ...
          <div data-testid="getPeople-test">
            <Content
              peoples={people.result}
              onClickButton={seeMorePeopleName}
